...
Посмотреть процессор в работе можно на примере проекта [эмулятора компьютера «Орион-128»](https://github.com/temaweb/orion). Оригинальный компьютер использовал советский аналог [КР580ВМ80А](https://en.wikipedia.org/wiki/KR580VM80A)
...
Шаблон может быть реализован как `uint16_t` для обмена данным с RAM или как `uint8_t` для обработки данных от инструкций `IN`/`OUT`. Обратите внимание, что во втором случае индекс устройства будет типа `uint8_t`
...
Длс логирования выполняемых инструкций, необходимо скомпилировать приложение с ключем `ASMLOG`. 
...
Расшифровка на примере последней строки:

|      | Описание |
| :---         |     :---      |
| `0x20C0` | Значение програмного счетчика |
| `CD 05 00` | Инструкция с аргументами. (`CALL 0x0005`) |
| `$D3` | Значение памяти по адресу аргумента (`0x0005`) |
| `CALL` | Название инструкции |
| `A:57` | Значение аккумулятора после выполнения инструкции |
| `B:00...L:00` | Значения регистров после выполнения инструкции |
| `$00 $00 $D3` | Значения памяти по адресам регистровых пар `BC`, `DE` и `HL` |
| `S=0` | Значение флага _Sign_ после выполнения инструкции |
| `Z=0` | Значние флага _Zero_ после выполнения инструкции |
| `AC=1` | Значение флага _Auxiliary Carry_ после выполнения инструкции |
| `P=0` | Значение флага _Parity_ после выполнения инструкции |
| `C=0` | Значение флага _Carry_ после выполнения инструкции |
| `0x2FED` | Указатель стека после выполнения инструкции |
...
После запуска, приложение выполняет несколько тестов для проверки работоспособности эмулятора. 
